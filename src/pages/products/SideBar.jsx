...
import { capitalizeFirstLetter, sortByPrice } from "utils";
    ...
    <div>
      <div className="px-10 py-9 mb-10 hidden flex-col gap-5 border-r md:flex">
        <div className="flex flex-col gap-4 border-b pb-4">
          ...
          {sections.map((i) => (
            <div className="bg-gray-100 rounded-lg text-black font-medium py-2 px-12" onClick={() => handleFilter(i)} key={i}>
        ...
        <Dropdown label="Filter" placement="right">
          ...
          <Dropdown inline={true} label="price" placement="right">
            <Dropdown.Item
              onClick={async () => {
                const sorted = await sortByPrice(category, 'asc');
                setProducts(sorted);
                }}>
              Ascending
            </Dropdown.Item>
            <Dropdown.Item
              onClick={async () => {
                const sorted = await sortByPrice(category, 'desc');
                setProducts(sorted);
                }}>
      ...
      <SideBarMenu>
        <div className="flex flex-col gap-4 border-b pb-4">
          <p className="block truncate text-lg font-medium">Select a category :</p>
          {sections.map((i) => (
            <div className="bg-gray-100 rounded-lg text-black font-medium py-2 px-12 flex justify-center" onClick={() => handleFilter(i)} key={i}>
              {capitalizeFirstLetter(i)}
            </div>
          ))}
        </div>
