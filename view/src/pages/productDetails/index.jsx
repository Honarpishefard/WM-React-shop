...
import {  fetchProductDetailsService, fetchProductsService, mediaURL } from "api";
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 sm:grid-cols-1 px-10 py-20 gap-10 flex-wrap justify-around">
          <img className="rounded-3xl justify-self-center max-h-40rem" src={mediaURL + product.image} alt="product image"/>
          <div className="pt-7 max-w-2xl">
            <p className="text-4xl text-gray-900 dark:text-white font-bold">{product.title}</p>
            <div className="flex gap-6 items-center py-5">
              <p className="text-2xl text-gray-900 dark:text-white font-medium">${product.newPrice}</p>
              <p className="text-xl text-gray-500 line-through dark:text-white font-medium">${product.oldPrice}</p>
            </div>
            <p className="leading-relaxed font-normal text-lg">{product.desc}</p>
            <div className="flex items-center px-6 py-10 justify-around">
              ...
              <Button onClick={() => handleAddToCard(product._id, size, quantity, cookieId)}>Add to card
      ...
      <div className="flex justify-center flex-col items-center">
        <p className="text-2xl text-gray-900 dark:text-white font-bold py-6 border-t">You may also like</p>
        <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 px-10 pb-14 pt-5 gap-10">
          {suggestedproducts?.map((i) =>
            loading ? (
              ...
              <Link to={`/products/${i.category[0]}/${i.category[1]}/${i._id}`} key={i._id}>
                <Card title={i.title} oldPrice={i.oldPrice} newPrice={i.newPrice} image={mediaURL + i.image}/>
              </Link>
            )
          )}
        </div>
