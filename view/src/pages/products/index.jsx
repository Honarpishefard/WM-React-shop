...
export const Products = () => {
  ...
  return (
    <>
      <Header />
      <div className="flex">
        {category === "men" ? (<SideBar sections={["jacket", "shoes", "tshirt"]} />
        ) : ( <SideBar sections={["coat", "dress"]} />
        )}
        <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-10 justify-center p-8">
          {products?.map((i) =>
            loading ? ( <CardSkeleton key={i._id} />
            ) : (
            <Card onClick={() =>  navigate(`/products/${i.category[0]}/${i.category[1]}/${i._id}`)} key={i._id} title={i.title} newPrice={i.newPrice}
              oldPrice={i.oldPrice} image={mediaURL + i.image}>
              <ModalComponent
